...
- [x] **Step 1: 전체 자동 테스트 실행**

Run: `npm test && npm run lint`
Expected: 전체 PASS, lint 에러 없음

완료 (160/160 테스트 통과, lint 에러 0개 — 이 플랜과 무관한 기존 lint 에러 1개도 같이 고쳐서
지금은 완전히 깨끗한 상태).
...
## 전체 구현 완료 후 최종 리뷰 (2026-09-23)

11개 태스크 구현·태스크별 리뷰가 전부 끝난 뒤, 전체 diff(6365329..HEAD)를 한 번 더 훑는
최종 리뷰를 돌렸다. 개별 태스크 리뷰에서는 안 보이던 것 3개를 찾음:

1. **반영 완료** — 이벤트를 삭제하면 `trigger_screenshots`에 있던 Storage 파일이 영구
   고아가 되는 문제. `removeEvent`에 삭제 전 Storage 정리 로직을 추가함 (커밋
   `ac49e26`).
2. **반영 완료** — `isAllowedAuthRedirect`(로그인 리다이렉트 허용목록 — 이번 플랜 중에도
   경계 버그를 한 번 고친 보안 관련 함수인데 테스트가 하나도 없었음)에 유닛 테스트 추가
   (같은 커밋 `ac49e26`).
3. **의도적으로 보류(범위 밖)** — Task 8에서 이미 기록해둔 `useProject`/`unwrap()` 이슈가,
   Studio와 기존 `/w/$wsId/p/$projectId` 라우트가 나란히 존재하게 되면서 더 뚜렷하게
   드러났다: 같은 "접근 거부" 상황에서 Studio는 정상적으로 "프로젝트를 열 수 없어요" 화면을
   보여주지만, 기존 라우트는 이 버그 때문에 깨진 빈 화면을 보여준다. 데이터가 새는 건
   아니고(RLS는 그대로 작동) 화면 표시 문제다. Task 8 때 "고치면 범위 밖의 기존 라우트
   동작이 바뀐다"는 이유로 보류했었고, 이번에도 같은 이유로 보류하되 — 이번엔 근거가 더
   뚜렷해졌으니 별도 후속 티켓으로 명확히 남겨야 한다.

**환경 제약으로 못 한 것 (Step 2, 3)**: 이 샌드박스엔 Docker/브라우저가 없어서 SQL 테스트
실행과 실제 클릭 테스트는 못 했다. 위 체크리스트는 Docker 있는 환경에서 사람이 직접 확인해야
한다.
