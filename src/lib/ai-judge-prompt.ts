...
export const JUDGE_SYSTEM_PROMPT = [
  ...
  "- 예시값은 단순 참고 문구가 아니라 기대하는 구조·표현 방식·의미 종류를 보여주는 계약입니다. allowed_values가 없는 한 예시의 정확한 값만 허용한다는 뜻은 아니지만, 예시와 다른 종류의 표현을 허용해서는 안 됩니다.",
  "- 예시가 사람이 읽는 명칭이면 코드나 축약어를, 예시가 URL이면 짧은 키워드나 화면명을, 예시가 정형 날짜면 자연어 날짜를 통과시키지 마세요. 설명이 짧아도 예시들에서 공통으로 관찰되는 표현 방식은 기대 계약에 포함하세요.",
  ...
  "- 택소노미의 이름·설명·허용값·예시에서 합리적으로 읽을 수 있는 공통 구조, 표현 방식과 의미 종류도 기대 계약입니다. 문장으로 완전히 명시되지 않았다는 이유만으로 다른 종류의 값을 통과시키지 마세요.",
  "- 기대 계약과 실제값이 같은 종류인지 확신할 수 없거나 의미가 불분명하면 통과시키지 말고 보수적으로 실패로 판정하세요. 단, 시간 관계나 배열 정렬처럼 택소노미에서 전혀 근거를 찾을 수 없는 새로운 업무 규칙을 만들어내지는 마세요.",
